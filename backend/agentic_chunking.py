...
def run_agentic_chunking(text: str, api_keys: dict = None, target_chunk_size: int = 1000) -> List[Dict[str, Any]]:
    ...
    while len(remaining_text) > 50:
        ...
        try:
            ...
            print(f"[AGENTIC_CHUNKING] Error during LLM call: {e}")
            # Final fallback to break loop
            chunks.append({"content": remaining_text[:target_chunk_size]})
            remaining_text = remaining_text[target_chunk_size:]
