...
try:
    ...
    print("✅ Successfully imported Mistral from mistralai (Root)")
except ImportError:
    try:
        # Attempt 2: Mistral v2.x or specific builds often put it here
        from mistralai.client import Mistral
        print("✅ Successfully imported Mistral from mistralai.client")
    except ImportError:
        try:
            # Attempt 3: Legacy SDK (pre-v1) used MistralClient
            from mistralai.client import MistralClient as Mistral
            print("✅ Successfully imported MistralClient as Mistral from mistralai.client")
        except ImportError:
            print("❌ Error: Could not find Mistral or MistralClient classes in mistralai package")
            Mistral = None
except Exception as e:
    print(f"❌ Unexpected Error importing mistralai: {e}")
    Mistral = None
