...
const SiteList = ({ onContextSelect, onWidgetCreate, onViewTimeline }) => {
        ...
        >
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              className="w-7 h-7 flex-shrink-0 flex items-center justify-center rounded-full text-slate-400 hover:bg-red-500 hover:text-white transition-colors"
            >
              ✕
            </button>
          </div>
...
function App() {
  const [view, setView] = useState('chat'); // 'chat' | 'settings' | 'history' | 'memory' | 'evolution' | 'research_path'
  ...
  const [showEvolutionUrl, setShowEvolutionUrl] = useState(null);
  const [showResearchSession, setShowResearchSession] = useState(null);
  ...
  const handleFileUpload = async (eventOrFile) => {
    ...
    if (file.type && !allowedTypes.includes(file.type) && !file.name.match(/\.(pdf|docx|csv|xls|xlsx|txt|md|json|png|jpg|jpeg|webp|mp4|webm|mp3|wav)$/i)) {
      toast.error('Unsupported file type. Please upload a PDF, Word, CSV, Text, Image, Audio, or Video file.');
      return;
    }
    ...
    <div className="flex flex-col bg-gradient-to-br from-slate-50 via-white to-indigo-50/50 text-sm font-sans antialiased text-slate-900 selection:bg-indigo-100 overflow-hidden max-w-full" style={{ position: 'fixed', inset: 0, height: '100dvh', width: '100vw' }}>
      ...
      {view === 'evolution' && showEvolutionUrl && (
        <div className="absolute inset-0 z-50 bg-white">
          <EvolutionTimeline
            sourceUrl={showEvolutionUrl}
            onCancel={() => {
              setView('memory');
              setShowEvolutionUrl(null);
            }}
          />
        </div>
      )}

      {/* Render Research Path */}
      {view === 'research_path' && showResearchSession && (
        <div className="absolute inset-0 z-50 bg-white">
          <ResearchPathMap
            sessionId={showResearchSession}
            onCancel={() => {
              setView('history');
              setShowResearchSession(null);
            }}
          />
        </div>
      )}
